package demo.craft.quickbooks.payroll.kafka.listener
...
class BusinessProfileValidationListener(
    ...
    properties: QuickbooksPayrollProperties
) {
    ...
    @KafkaListener(
        topics = ["\${demo.craft.quickbooks-payroll.kafka.businessProfileValidationRequestTopic}"]
    )
    fun onMessage(kafkaMessage: String) {
        ...
        val responsePayload = objectMapper.writeValueAsString(
            BusinessProfileValidationResponse(
                ...
                product = Product.QUICKBOOKS_PAYROLL,
                status = status,
