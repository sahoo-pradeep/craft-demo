...
publishing {
    publications {
        create<MavenPublication>("jar") {
            artifactId = "product-subscription-client"
            from(components["java"])
        }
    }
}

val clientPath = "$rootDir/services/product-subscription/api/client"
...
val generateClient by tasks.registering(GenerateTask::class) {
    ...
    inputSpec.set("$rootDir/services/product-subscription/api/spec.yml")
    outputDir.set(clientPath)

    // config
    generatorName.set("spring")
    library.set("spring-cloud")
    apiPackage.set("demo.craft.product.subscription.client.api")
    modelPackage.set("demo.craft.product.subscription.client.model")
    configOptions.set(
        mapOf(
